...
**Writers** (skills that create/update decision files): `/explore`, `/define-architecture`, `/define-slices`, `/create-plan`, `/complete-slice`

**Readers** (skills that load active decisions as context): all writers plus `/project-status`, `/start-project`, `/refine-plan`, `/implement-plan`
