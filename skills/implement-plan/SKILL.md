...
#### 3.0: Expected Behavior — RED Phase

Before implementation, execute the red half of the red-green cycle.

1. **Parse Expected Behavior**: Read the phase's Expected Behavior section. Extract all "Before implementation" checks.

2. **Set up fixtures** (if needed): Create test data, seed databases, write input files, start services — whatever the Before checks need to run against. Verification against empty state proves nothing.

3. **Run Before checks**: Execute each check. Record the output. Classify each result into one of these categories (they have different GREEN transition expectations):
   - **RED-CONFIRMED** — Check **ran and showed absence** (404, connection refused, test not found, grep returned 0, etc.). The check executed successfully and confirmed the behavior doesn't exist yet. GREEN expectation: re-run the same check — it should now pass.
   - **RED-INFRASTRUCTURE** — Check **couldn't execute** because infrastructure being built in this phase doesn't exist yet (e.g., server not started because this phase creates it, database not provisioned). The check itself couldn't run, not just the behavior is absent. GREEN expectation: the phase creates the infrastructure, then the check should both run AND pass.
   - **UNEXPECTED-PASS** — Check **unexpectedly passes** -> STOP. Present to user: "This check already passes: [check]. Either the phase scope is wrong or the check tests existing behavior. Options: (a) Remove this check — behavior already exists, (b) Revise the check to test something that doesn't exist yet, (c) Skip this phase." Wait for user response before proceeding.
   - **AGENT-BLOCKED** — Check **can't be run by the agent** (missing credentials, physical device, third-party system without access) -> pause. Present to user: "I can't run this check: [check]. Please run it and confirm the result." Wait for confirmation before proceeding.

4. **Resume detection**: If the phase's task checkboxes are partially checked (indicating a resumed implementation), Before checks may legitimately pass for already-completed work. Treat these as UNEXPECTED-PASS but with option (a) pre-selected — note the check passes due to prior work and proceed without user confirmation.

5. **Record RED results**: Note each check's category (RED-CONFIRMED, RED-INFRASTRUCTURE, UNEXPECTED-PASS, AGENT-BLOCKED) and output. This record is referenced after implementation to verify the GREEN phase — the category determines the expected transition.

**Non-blocking process rule**: See Verification Behavioral Rules (Step 6) rule #5.
...
5. **Run Verification section checks**: Execute any items in the phase's Verification section — broader integration/smoke checks that don't fit the before/after pattern (multi-phase integration concerns, performance checks, broader regression testing). This is for plan-author-specified checks only; automated suites (lint/build/test) are handled in sub-step 4. Follow the non-blocking process rule (Verification Behavioral Rules, rule #5).

#### 3.4: Expected Behavior — GREEN Phase

After implementation and post-phase verification, execute the green half of the red-green cycle.

1. **Re-run fixtures** if needed (services may need restarting after implementation changes).

2. **Run After checks**: Execute each item from the Expected Behavior "After implementation" section. Record the output.
   - Check **passes** -> record the success output as evidence. Proceed.
   - Check **fails** -> this is a bug. Do NOT proceed to commit. Feed the failure (including the check command and its output) back into the implementation-review cycle (Step 3.2) as a CRITICAL issue. The implementation agent must fix the failure and all After checks must pass before continuing.
   - Check **can't be run by the agent** -> pause. Present to user: "I can't run this check: [check]. Please run it and confirm the result." Wait for confirmation.

3. **Verify RED-to-GREEN transition**: For each check recorded in Step 3.0, confirm the expected transition:
   - **RED-CONFIRMED** checks: re-run — should now pass.
   - **RED-INFRASTRUCTURE** checks: should now both run AND pass (phase created the missing infrastructure). If still can't run, investigate — the phase should have created it.
   - **AGENT-BLOCKED** checks: ask user to re-verify.

4. **Record GREEN results**: Capture all verification output (command output, screenshots, etc.) as evidence. This goes in the phase completion report.

**Non-blocking process rule**: See Verification Behavioral Rules (Step 6) rule #5.

#### 3.5: Git Commit (MANDATORY)
...
### Verification Behavioral Rules

1. **The agent does its own verification.** If it can run a curl command, navigate a browser, execute a script, query a database — it does it. It does not list these as things for the user to do later.
2. **Pause-and-ask is the exception.** Only ask the user to verify when the agent genuinely cannot (missing credentials, physical hardware, third-party system without access).
3. **Verification is a gate, not a report.** A phase is not complete until After checks pass. There is no "here's what you should verify" at the end — it's already been done.
4. **Record the evidence.** Capture verification output (command stdout/stderr, screenshots, HTTP responses) as proof. This goes in the phase completion report and the final Verification Evidence table.
5. **Never block on a command that might not return.** Start dev servers, watch modes, and any persistent process in background. Verify with separate foreground commands (e.g., curl), then stop the process. If unsure whether a command will return promptly, run it in background.
