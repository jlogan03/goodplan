...
4. **Agent compatibility** — Are agent differences handled correctly? (Skip if single-variant skill.)
   ...
   Shared content (workflow logic, criteria, references) should be identical across variants. Flag agent-specific behavior leaking into shared reference files, or shared content unnecessarily duplicated across variants.
