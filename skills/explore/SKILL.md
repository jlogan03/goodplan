...
name: explore
description: >
  Runs an iterative research/brainstorm/prototype loop scoped to the project, a slice,
  or a side quest. Invoked when the user needs to investigate unknowns, explore options,
  or try approaches before committing to a plan or architecture. Common triggers: 'I need
  to research X', 'let's brainstorm', 'what are my options for...', 'let's explore',
  'what should I use for...', 'compare X vs Y', 'help me decide between...', 'I'm not
  sure which approach...', 'skip exploration'.
---
...
2. If it is a full path starting with `.project/` (e.g. `.project/vertical-slices/03-explore`), use as-is.
3. If it is a relative path like `vertical-slices/03-explore` or `side-quests/foo`, prepend `.project/`.
4. If it is a short name (e.g. `03-explore`), search for a match:

```bash
ls -d .project/vertical-slices/*"$SHORT_NAME"* .project/side-quests/*"$SHORT_NAME"* 2>/dev/null
```
...
1. **Work Stack top** — if the Work Stack has entries, take the top entry. Extract only the path portion (ignore any parenthetical metadata like `(interrupted ...)`). Use that as scope.
2. **Active Slice** — if Active Slice is set and not `none`, use that as scope.
3. **Project level** — otherwise, scope is project-level.
...
```bash
ls .project/vertical-slices/ .project/side-quests/ 2>/dev/null
```
...
1. Use the Read tool to load `~/.claude/skills/_shared/references/state-and-flow-formats.md` for state.md and flow-log.jsonl formats.
...
5. Update `.project/state.md` using the 4-section format from the shared formats reference. Set:
   ...
   - **Next Step**: appropriate recommendation (e.g., `/define-architecture` for project-level, `/create-plan` for slice)

6. Append to `.project/flow-log.jsonl`:

```bash
echo '{"ts":"<timestamp>","phase":"explore","scope":"<scope>","status":"complete","summary":"Exploration skipped: <reason>"}' >> .project/flow-log.jsonl
```

Scope value mapping: project-level uses `"project"`, slice uses `"vertical-slices/<name>"`, quest uses `"side-quests/<name>"`.
...
- **Project-level**: offer Research, Brainstorm, Prototype as three options
- **Slice or quest**: offer Research and Brainstorm only; add "(Prototype available at project-level scope only)" to the question text

On subsequent iterations, note previously used modes in the question text (e.g., "Pick a mode (already used: Research):").

If the user somehow selects Prototype at slice/quest scope, offer: "Want to switch to project scope for this prototype, or pick Research or Brainstorm instead?"
...
**Prototype** (project-level only): Ask for approach description, derive name, confirm with user, create directory, run interactive session, write `summary.md`. If the prototype directory already exists, append a numeric suffix (e.g., `auth-flow-2/`).
...
Use the Read tool to load `~/.claude/skills/_shared/references/state-and-flow-formats.md` for state.md and flow-log.jsonl formats.
...
- **Next Step**: appropriate recommendation based on scope:
  - Project-level: `/define-architecture`
  - Slice: `/create-plan`
