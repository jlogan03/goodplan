...
| `submit-plan` | `COMPLETE_PLAN` (or `COMPLETE_QUEST_PLAN`) — uses `--slice` or `--quest` to disambiguate |
| `submit-refinement` | `COMPLETE_REFINEMENT_ROUND` (or `COMPLETE_QUEST_REFINEMENT_ROUND`) — uses `--slice` or `--quest` to disambiguate |
| `submit-implementation` | `COMPLETE_IMPLEMENTATION` (or `COMPLETE_QUEST_IMPLEMENTATION`) — uses `--slice` or `--quest` to disambiguate |
...
Superseded by `goodplan state --json --query '.["activity-log.jsonl"] | ...'`. See `cli-changes.md` § What This Replaces.
...
goodplan state [--json] [--query <jq>] [--offset <n>] [--limit <n>]
goodplan init [--name <name>]
goodplan schema [--command <command-path>] [--json] [--query <jq>]
```

`state` exposes the full `assembleState()` tree as JSON. Read-only — routes directly to the Data Layer (no RPC or State Machine involvement). The tree structure mirrors the `.project/` directory layout: directories become objects, JSON files are unwrapped to their content, JSONL files become arrays, markdown files become strings. This is the public API for deep state access; `show`/`list`/`status` remain as ergonomic shortcuts for common queries.

| Flag | Type | Description |
|---|---|---|
| `--json` | boolean | Structured JSON output (default for `state`) |
| `--query` | string | jq filter applied server-side via jqjs — only selected data is serialized |
| `--offset` | number | Skip first N entries when `--query` returns an array; ignored otherwise |
| `--limit` | number | Return at most N entries when `--query` returns an array; ignored otherwise |
...
`schema` outputs the CLI's command tree with input/output schemas. Without `--command`, returns the full command hierarchy. With `--command` (e.g., `schema --command slice:complete`), returns that command's input schema, output schema, and flags. Output is always JSON (human-readable formatting by default, raw with `--json`). Supports `--query` for filtering. **Implementation note:** the `state` command must be added to the schema registry atomically with its implementation (INV-006).
...
|---|---|---|---|
| `--inline` | boolean or number | `start-*`, workflow `begin`/`complete` commands | Include inlined content in context bundles. `--inline` uses the default budget (~20-30KB); `--inline=<bytes>` overrides it. Invalid values (e.g., `--inline=abc`) must produce a `VALIDATION_INVALID_INPUT` error, not silently fall back to `true`. |
