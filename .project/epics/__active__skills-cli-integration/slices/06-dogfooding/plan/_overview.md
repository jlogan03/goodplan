...
- Two epics: Epic 1 (core provider + basic execution) exercises empty-state flows; Epic 2 (LLM-as-judge scoring) exercises non-empty-state flows with architecture proposal + `/start-epic`
- Side quests: at least one deliberate + organic ones from `/complete`
...
| 03 | side-quests | Exercise quest lifecycle (organic + deliberate) |
| 04 | second-epic-lifecycle | `/create-epic` on non-empty state, architecture proposal, `/start-epic`, full cycle |
