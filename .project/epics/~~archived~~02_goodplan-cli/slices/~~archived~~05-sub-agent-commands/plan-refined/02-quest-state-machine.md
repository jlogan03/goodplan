...
- [x] Create `src/core/state/transitions/quest-abandon.ts` — ABANDON_QUEST handler. Guard: status is non-terminal (not completed, not abandoned). Sets abandoned, records reason. Calls `updateQuestOverviewStatus`. Clears activeQuest if this was the active quest. Appends activity-log.
- [x] Update `transition-tables.md`: set `BEGIN_QUEST_PLAN` guard to `activeQuest == null` (currently shows "—"), and add a "one-active-quest" entry to the Cross-Cutting Guards table. This is the source-of-truth update for the guard implemented in `quest-plan.ts`.
- [x] Replace Phase 1's placeholder stubs in `reduce.ts` handler record with real handler imports.
- [x] Export transition tables from each handler file as typed arrays.
- [x] Write unit tests organized by handler file:
