...
- [ ] Update the `satisfies` handler record in `reduce.ts` — add placeholder entries for the 6 new quest event types. Create a `handleNotImplemented` helper (one-liner returning `{ ok: false, code: 'STATE_INVALID_TRANSITION', message: 'Not yet implemented' }`) or use inline arrow functions returning the same StateError shape.
