...
- [ ] Create `src/core/state/transitions/epic-phase.ts` — BEGIN/COMPLETE handlers for the explore→architecture→slicing phase chain. BEGIN_EXPLORE: guard status == created, set exploring. COMPLETE_EXPLORE: two from-status matches per transition-tables.md — `from: created → explored` (skip path) and `from: exploring → explored` — each unconditional (no business-logic guard). Not a single handler with an OR guard. BEGIN_ARCHITECTURE through COMPLETE_SLICING: follow transition-tables.md exactly for from-status guards and to-status transitions. Each handler updates epic.json status + appends activity-log entry.
