...
- [ ] Write unit tests: `begin(projectDir, 'create', {type:'slice', name}, {name, goal, epic})` creates slice, `begin(projectDir, 'plan', {type:'slice', name})` transitions, `begin(projectDir, 'abandon', {type:'slice', name}, {reason})` with reason, `complete(projectDir, {type:'slice', name}, input)` with full payload (deferred, learnings, architectureDelta), complete with verificationPassed:false → error
