...
# Decisions matching a specific domain
goodplan state --json --query '[.["decisions.jsonl"][] | select(.domain == "architecture")]'
...
### State Tree JSON Serialization Format

When serialized to JSON (via `goodplan state --json`), `StateEntry` variants are **unwrapped** — the `type` wrapper is stripped so the tree reads naturally as a JSON document:

| Internal type | JSON serialization | Example |
|---|---|---|
| `DirectoryEntry` | Plain object — keys are child names, values are serialized children | `{ "epic.json": {...}, "research": {...} }` |
| `JsonEntry<T>` | Unwrapped — serializes as `T` directly (no `{ type: "json", content: T }` wrapper) | `{ "name": "my-epic", "status": "activated", ... }` |
| `JsonlEntry<T>` | Unwrapped — serializes as `T[]` directly | `[{ "ts": "...", "phase": "init", ... }]` |
| `MarkdownEntry` | Unwrapped — serializes as a raw JSON string | `"# Plan\n\nThis slice implements..."` |

This means jq queries navigate a clean tree: `.slices["my-slice"]["slice.json"].status` returns the status string directly, not `.slices["my-slice"]["slice.json"].content.status`.

The root `ProjectState` (a `DirectoryEntry`) is serialized as a plain object — the outermost `{ type: "directory", contents: {...} }` wrapper is also stripped, so the top-level keys are `project.json`, `activity-log.jsonl`, `epics`, `slices`, etc.
...
// Added to slice:show --json response
{
  ...
  artifacts: {
    goal: boolean;           // goal field exists in entity JSON (not a separate file)
...
## 4. Semantic Versioning and Compatibility Checking
...
| Condition | Behavior |
|-----------|----------|
| CLI major > data major | Warn: "Project data was created with an older major version." Proceed with best effort. |
...
Schema migration (`goodplan migrate`) is deferred to a future epic. The current major version (1.x) does not require migration infrastructure. When major versions change, a dedicated migration epic will define:
- Migration registry pattern
- Pure migration functions: `(oldState: ProjectState) → newState: ProjectState`
- Compatibility checking behavior for major version mismatches
...
4. **`--version --json`** — required for skill version compatibility checking
