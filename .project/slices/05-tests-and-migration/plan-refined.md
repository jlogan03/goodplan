...
- [ ] Back up `.project/` state: `cp -r .project .project-backup` (belt-and-suspenders beyond `.project-old-<timestamp>/`)
- [ ] Verify backup: `ls .project-backup/project.json` → exists
...
- [ ] Clean up: remove `.project-backup/` after verification (`.project-old-<timestamp>/` is kept as the standard backup)
