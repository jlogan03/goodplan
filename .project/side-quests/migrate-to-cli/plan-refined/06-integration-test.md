...
- [ ] Migration has been run successfully on a copy of this repo's `.project/`:
  ...
  - `goodplan epic:list --json` lists all discovered epics with inferred statuses (use relative assertions — don't hardcode specific epic names or counts as repo state may change)
  - `goodplan quest:list --json` lists all discovered quests with appropriate statuses
...
- [ ] **Write automated integration test** at `tests/integration/migrate.test.ts`:
  ...
  - Run full migration flow programmatically:
    ...
    2. Construct valid answers matching the fixture — use `z.infer<typeof inventoryResponseSchema>` (and equivalent for other round schemas) to type programmatic answers, ensuring test data stays in sync with schema changes
  ...
  - **Unit tests for `buildMigrationState()`** (the `@internal` named export from `src/core/rpc/migrate.ts`, extracted in Phase 4):
    - Given validated inventory + epic details → produces correct `ProjectState` shape
    - Correctly populates per-status required fields (timestamps, sliceSequence)
    - Handles edge cases: epic with zero slices, quest with only goal
  - Test error cases:
    - Invalid sourcePath → validation error
    - Already-migrated project → `STATE_ALREADY_INITIALIZED`
    - No `.project/` → `DATA_NO_PROJECT`
