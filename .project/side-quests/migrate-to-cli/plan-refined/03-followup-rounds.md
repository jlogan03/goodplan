...
- [x] After submitting Round 1 answers, the CLI returns Round 2 questions but submitting Round 2 answers results in an error or incomplete behavior

**After implementation** (should pass / show presence):
- [x] After Round 1 (inventory): CLI emits Round 2 with one question per epic asking for slice details, architecture presence, activation date, and slice sequencing
- [x] After Round 2 (epic details): CLI emits confirmation round with full state summary
- [x] Confirmation with `approved: true` → CLI returns `{ "status": "complete", "summary": {...} }` (Phase 4 will handle actual state construction)
- [x] Confirmation with `approved: false, reAnswerIds: ["epic-details-initial"]` → CLI re-emits only the specified question(s)
- [x] After 3 correction rounds without approval → CLI throws `GoodplanError` with code `VALIDATION_MIGRATION_CORRECTION_LIMIT` including summary of what was answered so far
- [x] All `sourcePath` fields in follow-up answers are validated immediately

### Tasks

- [x] Implement Round 2 question generation — driven by Round 1's epic inventory:
...
- [x] Implement confirmation round generation:
...
- [x] Implement re-answer correction protocol:
...
- [x] Update `.migration-in-progress.json` schema to track (aligned with the canonical `MigrationState` shape from Phase 1):
