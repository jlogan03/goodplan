...
- [x] **Extend Step 0 — Scope Resolution**: Add variable setup for initiative scope (Step 2 handles detection). When Step 2 determines `$SCOPE_TYPE = initiative`, set `$INITIATIVE_DIR` to the matched `initiatives/__active__*/` path, `$SLICES_DIR = $INITIATIVE_DIR/vertical-slices/`. Load `initiative-conventions.md` for archive numbering and completion conventions.

- [x] **Extend Step 2 — Determine Scope**: Add initiative detection to scope resolution (Step 2 detects, Step 0 sets variables). An initiative scope matches `initiatives/__active__*/` without a trailing `vertical-slices/<slice>/` path. When scope is an initiative:
...
- [x] **Extend Step 3 — Load Artifacts**: For initiative scope, load:
...
- [x] **Extend Step 4 — Synthesize Learnings (initiative variant)**: For initiative scope:
...
- [x] **Extend Step 5 — Roll Up to Top-Level Learnings (initiative variant)**: For initiative scope, roll up initiative-level learnings to `.project/learnings.md` using `_Source: <initiative-name>_` tag. Idempotency check as usual.

- [x] **Extend Step 6 — Architecture Reconciliation (initiative variant)**: For initiative scope, reconcile the two layers:
...
- [x] **Add artifact promotion step** (new Step 6e, after signal tracking): For initiative scope only:
...
- [x] **Handle intermediate steps for initiative scope**: Explicit decisions for steps that don't naturally apply:
...
- [x] **Extend Step 8 — Review Remaining Work (initiative variant)**: For initiative scope, instead of reviewing remaining slices within the initiative (all are done), review the broader project:
...
- [x] **Extend Step 10 — State Update + Flow Log (initiative variant)**: For initiative scope:
...
- [x] **Extend Step 10b — Archive with Numbering (initiative variant)**: For initiative scope:
...
- [x] **Update graceful stop**: Add initiative-specific partial states. Note: Phase 1's cross-reference update already renamed existing cases (a)-(d) from `complete-slice` to `complete`; these new cases extend the set.
...
- [x] **Update guidance.md**: Add "Initiative Completion Protocol" section covering:
