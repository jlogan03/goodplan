...
- [ ] **Update shared preamble** (`~/.claude/skills/refine-plan/references/shared-preamble.md`): In the Codebase Exploration section, add: "If `architecture/invariants.md` exists, read it — the document under review must not violate documented system invariants without explicit justification and an amendment step."
...
- Verify the same reviewer files are also used by `/refine-slices` and `/implement-plan`:
  - `reviewers-cross-cutting.md` changes automatically apply to `refine-plan`, `refine-slices`, and `implement-plan` (all three use it)
