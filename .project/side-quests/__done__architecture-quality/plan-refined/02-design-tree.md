...
**Tracking**: In-memory during the conversation. The agent presents progress as a compact table: "Branch | Status (resolved/open/deferred) | Decision ref". Resolved decisions are written to `.project/decisions/`. The architecture files capture the final resolved state — no separate design-tree.md artifact. **Persistence limitation**: branch resolution state is lost on context compaction or fresh session. This is an acceptable limitation — the written decisions in `.project/decisions/` serve as the durable record, and the deep pass can reconstruct context from them.

### Tasks

- [x] Read current `~/.claude/skills/define-architecture/SKILL.md` to understand the existing flow
- [x] Restructure the skill flow. Current steps become:
...
- [x] Write Step 5 — Design tree (broad pass):
  ...
  6. Stop when subsystem boundaries, communication patterns, key constraints, and data ownership are all resolved — this is the minimum structure needed for design-it-twice to generate meaningfully different options.
...
- [x] Write Step 7 — Design tree (deep pass):
...
- [x] Update graceful stop handling (Step 5e equivalent) to cover the new steps:
...
- [x] Create a reference file `~/.claude/skills/define-architecture/references/design-tree.md` containing the detailed design tree interaction protocol. **SKILL.md vs reference file split**:
  - **SKILL.md Steps 5 and 7 keep**: numbered substep titles and one-line descriptions (the 7 substeps listed above for each pass). This gives the agent the sequence and intent at a glance.
  - **Reference file contains**: the detailed interaction protocol for each substep — branching logic (when to pursue a follow-up vs defer), progress display format (table columns, status values, update frequency), stopping criteria for broad pass (concrete: subsystem boundaries, communication patterns, key constraints, and data ownership all resolved) vs deep pass (all branches resolved or explicitly deferred), expertise calibration guidance for tree-walking, and decision deduplication (check existing `.project/decisions/` for overlap before writing a new one — supersede or skip).
  - SKILL.md references the file with: "See `references/design-tree.md` for the full interaction protocol."
- [x] Update the skill's reference loading (Step 1) to include the new `design-tree.md` reference file. Note: `design-tree.md` and `design-it-twice.md` are only needed during Steps 5-7; consider loading them on-demand at their respective steps rather than unconditionally at Step 1, for future maintainability as the reference file count grows
- [x] Renumber all subsequent steps and update all internal step references

### Verification

- Read updated `define-architecture/SKILL.md` — confirm Steps 5 and 7 exist with design tree logic
- Read `define-architecture/references/design-tree.md` — confirm full interaction protocol exists
