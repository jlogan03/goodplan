...
**Key decisions**:
- Deep module criteria added as new items (8-11) to the existing Software Architecture reviewer, not integrated into existing criteria
...
- `/refine-architecture` is a separate skill (not a mode of `/refine-plan`), using the same iteration loop with fewer reviewers
- `/audit-architecture` has two functions: gap analysis (architecture vs code) and architecture reassessment (should the target change?)
...
| 04 | Refine-architecture skill | New skill using reviewer infrastructure to iteratively review architecture files |
| 05 | Audit-architecture skill | New skill comparing architecture vs code, evaluating architecture quality, proposing side quests |
