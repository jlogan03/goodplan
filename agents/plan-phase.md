...
4. **Write output**: Write the plan to the temp working directory path specified in your task prompt (e.g., `<tmpdir>/draft/plan.md` for single-file or `<tmpdir>/draft/plan/` for directory format). Use directory format if the plan exceeds ~300 lines.
...
If you cannot complete the plan (missing critical context, ambiguous goal), return:
