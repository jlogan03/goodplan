...
**Tool access**: Read, Grep, Glob, Write, Bash, WebSearch (fall back to codebase exploration if WebSearch unavailable). **Disallowed**: Agent (no sub-agent spawning). Tool restrictions are enforced by the spawning orchestrator via `allowedTools`/`disallowedTools` in the agent invocation.
...
@${CLAUDE_PLUGIN_ROOT}/skills/_shared/references/expertise-tracking.md
@${CLAUDE_PLUGIN_ROOT}/skills/_shared/references/maturity-conventions.md
@${CLAUDE_PLUGIN_ROOT}/skills/_shared/references/codebase-context-discovery.md
...
Write `.goodplan/idea.md` directly using the Write tool:
...
1. **Naming conventions** — sample up to 20 `src/` files (prioritizing files with most git commits). Detect file naming pattern (kebab-case, camelCase, PascalCase, snake_case), export naming, variable naming.
...
Write `.goodplan/conventions.md` directly using the Write tool. Sections:
...
Assign maturity per the maturity-conventions reference: Experimental, Developing, Maturing, or Foundational.
...
Write `.goodplan/architecture/_overview.md` using the Write tool:
...
The Project Context section should reference:
