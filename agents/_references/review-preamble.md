...
The `score` field is the integer score from your `## Score` section. The orchestrator uses this for exit condition evaluation. The `review` field contains your complete review text — the orchestrator writes it to the appropriate file in `<tmpdir>/reviews/`. Do not attempt to write review files yourself.
