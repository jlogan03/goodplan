...
/**
 * `gp start-architecture --epic <name> [--inline[=<bytes>]]`
 *
 * Read-only context assembly for the architecture phase. Returns ContextBundle JSON.
 * Always outputs JSON regardless of --json flag (sub-agent command).
 */
export const startArchitectureCommand = defineCommand({
	meta: {
		name: "start-architecture",
		description: "Assemble context bundle for architecture phase. Requires --epic.",
	},
	args: {
		...globalArgs,
		epic: {
	...
	run({ args }) {
		...
		const bundle = startContext(state, "architecture", { type: "epic", name: args.epic as string }, options);
