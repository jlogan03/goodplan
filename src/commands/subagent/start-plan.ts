 ...
 */
export const startPlanCommand = defineCommand({
	meta: {
		name: "start-plan",
		description: "Assemble context bundle for plan phase. Requires --slice or --quest.",
	},
	args: {
		...globalArgs,
		slice: {
	...
	run({ args }) {
		...
		const bundle = startContext(state, "plan", target, options);
