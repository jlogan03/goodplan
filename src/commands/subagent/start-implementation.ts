...
/**
 * `gp start-implementation --slice <name>|--quest <name> [--inline[=<bytes>]]`
 *
 * Read-only context assembly for the implementation phase. Returns ContextBundle JSON.
 * Always outputs JSON regardless of --json flag (sub-agent command).
 */
export const startImplementationCommand = defineCommand({
	meta: {
		name: "start-implementation",
		description: "Assemble context bundle for implementation phase. Requires --slice or --quest.",
	},
	args: {
		...globalArgs,
		slice: {
	...
	run({ args }) {
		...
		const bundle = startContext(state, "implementation", target, options);
