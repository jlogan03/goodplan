...
/**
 * `gp start-slices --epic <name> [--inline[=<bytes>]]`
 *
 * Read-only context assembly for the slices phase. Returns ContextBundle JSON.
 * Always outputs JSON regardless of --json flag (sub-agent command).
 */
export const startSlicesCommand = defineCommand({
	meta: {
		name: "start-slices",
		description: "Assemble context bundle for slices phase. Requires --epic.",
	},
	args: {
		...globalArgs,
		epic: {
	...
	run({ args }) {
		...
		const bundle = startContext(state, "slices", { type: "epic", name: args.epic as string }, options);
