...
import { activateInvariantInputSchema } from "../../schemas/commands/invariant.js";
...
/**
 * `gp invariant:activate` — activate a proposed custom invariant.
 *
 * Stdin: { "id": "custom.no-large-files" }
 */
export const invariantActivateCommand = defineCommand({
	meta: {
		name: "invariant:activate",
		description: "Activate a proposed custom invariant. Accepts stdin JSON { id }.",
	},
	args: {
		...globalArgs,
	},
	...
	async run({ args }) {
		...
		const parsed = activateInvariantInputSchema.safeParse(stdin);
		...
		if (existing.status === "active") {
			const errorOutput = {
				ok: false,
				error: `Invariant '${parsed.data.id}' is already active`,
				code: "ENTITY_ALREADY_ACTIVE",
			};
			if (args.json || args.query) {
				output(errorOutput, args);
			} else {
		...
		try {
			const result = await appendEvent({
				...
				scope: "project",
				scopeRef: null,
				actor: { kind: "cli", id: "gp:invariant:activate" },
				branch: ctx.branch,
			...
			if (args.json || args.query) {
				...
				output(`Activated invariant ${pc.bold(parsed.data.id)}`, args);
			}
		} catch (error) {
