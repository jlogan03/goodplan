...
/**
 * `gp slice:implement-start --epic <name> --slice <name>` (v2)
 *
 * Emits `slice-implementation-started` with domain "entity-lifecycle".
 ...
 */
export const sliceImplementStartCommand = defineCommand({
	meta: {
		name: "slice:implement-start",
		description: "Start implementation of a slice.",
	},
	args: {
		...globalArgs,
		epic: {
	...
	async run({ args }) {
		...
		try {
			const result = await appendEvent({
				...
				actor: { kind: "cli", id: "gp:slice:implement-start" },
				branch: ctx.branch,
			...
			if (args.json || args.query) {
				...
				output(
					`Implementation started for slice ${pc.bold(ctx.sliceName)} in epic ${pc.bold(ctx.epicName)}`,
					args,
				);
			}
		} catch (error) {
