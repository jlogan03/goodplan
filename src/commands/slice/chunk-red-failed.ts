...
/**
 * `gp slice:chunk-red-failed --epic <name> --slice <name> --chunk <id>` (v2)
 *
 * Accepts stdin JSON `{ evidence }`.
 * Emits `chunk-red-test-failed` with domain "entity-lifecycle".
 */
export const sliceChunkRedFailedCommand = defineCommand({
	meta: {
		name: "slice:chunk-red-failed",
		description: "Record that a red test has failed (expected) for a chunk.",
	},
	args: {
		...globalArgs,
		epic: {
	...
	async run({ args }) {
		...
		try {
			const result = await appendEvent({
				...
				actor: { kind: "cli", id: "gp:slice:chunk-red-failed" },
				branch: ctx.branch,
			...
			if (args.json || args.query) {
				...
				output(
					`Red test failed (confirmed) for chunk ${pc.bold(chunkId)} in slice ${pc.bold(ctx.sliceName)}`,
					args,
				);
			}
		} catch (error) {
