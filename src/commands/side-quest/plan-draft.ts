...
/**
 * `gp side-quest:plan-draft` — draft a plan for a side-quest.
 *
 * Stdin: { plan: ContentRef }
 * Emits `side-quest-plan-drafted` event.
 */
export const sideQuestPlanDraftCommand = defineCommand({
	meta: {
		name: "side-quest:plan-draft",
		description: "Draft a side-quest plan. Stdin: {plan: ContentRef}.",
	},
	args: {
		...globalArgs,
		...sideQuestArgs,
	...
	async run({ args }) {
		...
		try {
			const result = await appendEvent({
				...
				actor: { kind: "cli", id: "gp:side-quest:plan-draft" },
				branch: ctx.branch,
				commitHint: ctx.commitHint,
				domain: "entity-lifecycle",
				type: "side-quest-plan-drafted",
			...
			if (args.json || args.query) {
				...
				output(`Drafted plan for side-quest ${pc.bold(ctx.sideQuestName)}`, args);
			}
		} catch (error) {
