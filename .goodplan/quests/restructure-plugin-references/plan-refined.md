...
**Before implementation** (should fail / show absence):
- [ ] Reviewer agents reference review criteria at `skills/_references/review-*.md` (not yet at `agents/_references/`)
- [ ] No `workflow-guide` skill exists in `skills/`
- [ ] Skills `explore`, `status`, `upgrade` instruct LLM to use Read tool for their local references
- [ ] `create-epic` and `plan-slice` instruct LLM to Read `skills/implement/references/reviewer-registry.md`
- [ ] `reviewer-registry.md` lives under `skills/implement/references/` (not yet at `skills/_references/`)

**After implementation** (should pass / show presence):
- [ ] Every `@` reference in `agents/*.md` resolves to an existing file at the referenced path
- [ ] Every `@` reference in `skills/*/SKILL.md` resolves to an existing file at the referenced path
- [ ] No skill or agent instructs the LLM to use Read tool to load always-needed reference/instruction files (intentionally deferred/conditional Read instructions are preserved)
- [ ] `skills/_references/` contains files consumed by skills and cross-consumer shared files (agents reference these via `@${CLAUDE_PLUGIN_ROOT}/skills/_references/`)
- [ ] `agents/_references/` contains only files consumed exclusively by agents
- [ ] No root `_references/` directory exists — two tiers only: `skills/_references/` and `agents/_references/`. Cross-consumer files go in `skills/_references/` (agents already reference via `@${CLAUDE_PLUGIN_ROOT}/skills/_references/`)
- [ ] Unused files are deleted (README.md, state-and-activity-formats.md, others identified in Phase 1)
- [ ] Review criteria files (review-preamble.md + 20 review-*.md) are moved to `agents/_references/` with existing `@` include pattern preserved in reviewer agent definitions
- [ ] `skills/workflow-guide/SKILL.md` exists with `user-invocable: false`, containing CLI orientation, state interaction rules, workflow entry points, and flow recovery guidance
- [ ] All skills that previously loaded CLI interaction rules via Read or @ retain an explicit `@` include of `cli-interaction.md` — workflow-guide supplements (does not replace) these explicit includes. `cli-interaction.md` provides the operational rules each skill needs; `workflow-guide/SKILL.md` provides orientation context (entry points, flow recovery) auto-loaded via description matching. Skills do not need to `@`-include workflow-guide
- [ ] `reviewer-registry.md` lives at `skills/_references/reviewer-registry.md` and all consumers reference it via `@` include (no cross-skill Read calls)

### Tasks

- [ ] Create `agents/_references/` directory
- [ ] Create `skills/workflow-guide/SKILL.md` with `user-invocable: false` — content boundary: CLI query/mutation patterns, .goodplan/ write restrictions, skill entry points per flow, interrupted flow recovery, essential error handling. Explicitly excludes: per-skill operational logic (stays in skill SKILL.md), review criteria (stays in agents/_references/), and reference content already covered by `@` includes in consuming skills. Source content from `cli-interaction.md` sections covering CLI patterns, state restrictions, and workflow routing — not the full 28KB file
- [ ] Move agent-only reference files to `agents/_references/` (sub-agent-return-format.md, audit-conventions.md, codebase-context-discovery.md, maturity-conventions.md, plan-format.md — subject to Phase 1 audit confirming these are agent-only)
- [ ] Move review-preamble.md + all review-*.md to `agents/_references/`; keep the existing `@` include pattern (do not inline — 21 files totaling ~80KB would create massive duplication)
- [ ] Move `reviewer-registry.md` from `skills/implement/references/` to `skills/_references/`
- [ ] Evaluate `plugin/CLAUDE.md` vs `workflow-guide` coexistence: if workflow-guide subsumes `plugin/CLAUDE.md` content, delete it and remove the build script copy step; if distinct, document why both exist
- [ ] Place any cross-consumer files in `skills/_references/` (agents reference via `@${CLAUDE_PLUGIN_ROOT}/skills/_references/`); no root `_references/` directory
- [ ] Update all `@` paths in agent definitions for moved files
- [ ] Update `@` paths in reviewer agent definitions for moved review files
- [ ] Update `iteration-loop.md` line 48 and all other consumers to reference `skills/_references/reviewer-registry.md` via `@` include
- [ ] Convert Read tool instructions in explore, status, upgrade SKILL.md files to `@` auto-includes for their local `references/*.md` files — but only for always-loaded top-level Read instructions. Preserve conditional/deferred Read for actual skill reference files only: `explore/references/explore-logic.md` (on-demand in Step 1), `status/references/status-logic.md` (conditional loading), `upgrade/references/migration-heuristics.md` (loaded in Step 4 only when answering questions). Note: project-data Reads (e.g., `.goodplan/` sequencing/expertise files) are out of scope for this restructuring
- [ ] Convert `implement` SKILL.md's inline `@` reference to iteration-loop.md to a mandatory standalone `@` line (not embedded in prose). Accept the ~300-line context cost — Phase 3's loop consolidation depends on this reference loading reliably, and silent failure is the worst case
- [ ] Verify build passes with new layout
- [ ] Delete files identified as unused in the Phase 1 audit (including `state-and-activity-formats.md` — confirmed zero consumers) — deletions intentionally follow build verification so breakage is caught before removing files
- [ ] Re-verify build passes after deletions. Diagnostic: first build (after moves) should succeed; second build (after zero-consumer deletions) should also succeed. If second fails, the Phase 1 audit missed a consumer
...
**Before implementation** (should fail / show absence):
- [ ] Reference files contain content not used by any consumer (migration history, deprecated patterns, obvious examples)
- [ ] `plan-slice`, `create-epic`, `create-side-quest` define iteration loops inline instead of referencing iteration-loop.md
- [ ] Reference files may duplicate content that's in the workflow-guide skill

**After implementation** (should pass / show presence):
- [ ] Every section in every reference file is used by at least one consumer
- [ ] No reference file duplicates content in the workflow-guide skill
- [ ] Where consumers use disjoint sections of a reference, the file is split so each loads only its portion
- [ ] No contradictions between reference files, between references and skill instructions, or between references and the workflow-guide skill
- [ ] All reference files are complete and correct — no consumer lost information it needs
- [ ] `plan-slice`, `create-epic` (both arch and slices loops), and `create-side-quest` reference `iteration-loop.md` with skill-specific Loop Parameters instead of reimplementing inline
- [ ] `iteration-loop.md` is complete enough to support all four consuming skills' loop patterns

### Tasks

- [ ] For each reference file: remove sections no consumer uses (based on Phase 1 audit)
- [ ] For each reference file: remove content that now lives in the workflow-guide skill
- [ ] Where a reference serves multiple consumers with disjoint needs, split into focused files and update `@` paths
- [ ] Review all references for contradictions; resolve them
- [ ] Update `iteration-loop.md` to be complete and parameterizable for all four loop patterns (implement, plan-refinement, architecture-refinement, slices-refinement). The shared loop is scoped to **iteration control flow only** — exit criteria, stagnation detection, round directory management. Reviewer context assembly and score computation stay inline per skill (these diverge too much to parameterize cleanly). The Loop Parameters schema covers:
...
- [ ] Replace inline loop implementations in `plan-slice/SKILL.md` with reference to iteration-loop.md + Loop Parameters section. Each consuming skill includes a `## Loop Parameters` section in the same format as `implement/SKILL.md`
- [ ] Replace inline loop implementations in `create-epic/SKILL.md` (both architecture and slices refinement) with references to iteration-loop.md + Loop Parameters sections
- [ ] Replace inline loop implementation in `create-side-quest/SKILL.md` with reference to iteration-loop.md + Loop Parameters section
- [ ] Verify completeness: for each consumer, confirm it still has access to all information it needs
...
| create-epic (arch) | [5, 7, 9] | Exit at round 3 | max_iterations: 3 reached |
| create-epic (slices) | [8, 7, 6] | Exit at round 3 | reduction_exit_threshold: 2 total decreases (rounds 2-3) |
| create-side-quest | [6, 8, 9, 9, 9] | Exit at round 5 | stagnation_window: 2 consecutive no-change rounds (rounds 4-5: both 9) |
...
**Before implementation** (should fail / show absence):
- [ ] Build script assertions reference old directory structure

**After implementation** (should pass / show presence):
- [ ] `bun run build` succeeds with no errors
- [ ] Build script assertions updated for new directory structure (new shared directories, workflow-guide skill, removed files)
- [ ] All `@` references in built `dist/gp-plugin/` resolve to existing files
- [ ] Dogfood test harness runs on Opus with `bypassPermissions` and asserts zero `Read` tool calls targeting shared reference paths (`skills/_references/`, `agents/_references/`) — preserved conditional Reads for skill-local references (explore-logic.md, status-logic.md, migration-heuristics.md) are expected and excluded from this assertion
- [ ] Dogfood test demonstrates correct LLM behavior: skills load restructured references and execute without errors (behavioral correctness beyond reference loading is verified by existing dogfood tests, not this restructuring-specific test)

### Tasks

- [ ] Update `scripts/build-plugin.sh`: adjust assertions for new directory structure — use exact `-eq 13` skill count assertion with a comment listing expected skills (exact counts catch accidental additions; the deleted-skills guard handles regressions), reference directory checks, removed file guards
- [ ] Verify no root `_references/` directory exists (two-tier layout: `skills/_references/` and `agents/_references/` only)
- [ ] Confirm `agents/_references/` is present in dist after existing agents rsync (rsync -a handles subdirectories). Add assertion: `test -d "$PLUGIN_DIR/agents/_references/"` (parallels existing `skills/_references/` check)
- [ ] Add brief comment in `build-plugin.sh` explaining the two-tier layout: `skills/_references/` (skill-shared + cross-consumer) vs `agents/_references/` (agent-only, primarily review criteria). Phase 4 verification: `grep -q 'two-tier' scripts/build-plugin.sh` confirms the comment exists
- [ ] Run `bun run build` and fix any failures
- [ ] Replace the existing agents-only `@` reference validation loop in `scripts/build-plugin.sh` (currently lines 190-215) with a single unified loop using glob `$PLUGIN_DIR/{skills,agents}/**/*.md` — this replaces, not supplements, the existing loop to avoid double-validating agent references
- [ ] Verify all `@` references in `dist/gp-plugin/` resolve via the extended validation
- [ ] Create or adapt dogfood test that exercises key flows on Opus with `bypassPermissions`: status query, plan creation (to test iteration loop), skill invocation (to test reference loading)
- [ ] Assert zero `Read` tool calls targeting shared reference paths (`skills/_references/`, `agents/_references/`) in the tool call log — this verifies shared reference loading uses `@` auto-includes. Preserved conditional Reads for skill-local references are expected and excluded
...
- [ ] If test reveals broken instructions or missing context, fix in source and re-run
