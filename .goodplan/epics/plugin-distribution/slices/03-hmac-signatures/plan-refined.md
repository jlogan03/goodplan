...
- [ ] Modify `loadState()` in `src/core/data/load.ts`:
...
- [ ] Add `__GP_HMAC_KEY__: JSON.stringify("goodplan-dev-hmac-key")` to `vitest.config.ts`'s `define` map (unit tests import source files through Vitest's module transform, not the compiled binary)
