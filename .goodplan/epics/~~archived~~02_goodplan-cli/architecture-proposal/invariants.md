...
- **Verification:** Resource commands are read-only. All mutation commands route through the RPC Layer which calls `reduce()`. Fitness function candidate: no direct `writeEntity()` calls outside of `commitState()` for entity JSON files.
...
## INV-006: No silent errors — structured error responses with correct exit codes
...
- **Verification:** Exit code 0 on success, 2 for invalid CLI usage (validation errors), 1 for all other errors. All errors return the structured `{ error: { code, message, detail? } }` shape. No empty catch blocks. Fitness function candidate: test confirming every error code path produces a non-zero exit and a valid error JSON object.
