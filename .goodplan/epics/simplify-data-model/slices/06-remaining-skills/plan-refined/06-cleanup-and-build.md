...
- [x] `ls skills/ | grep -v _shared | wc -l` — returns more than 12
- [x] `ls scripts/install-skills.sh` — file exists (should be deleted)

**After implementation** (should pass / show presence):
- [x] `ls skills/ | grep -v _shared | wc -l` — returns exactly 12
...
- [x] Delete 15 old skill directories:
...
- [x] Delete `scripts/install-skills.sh` — superseded by plugin distribution via `build-plugin.sh`

- [x] Update `build-plugin.sh`:
...
- [x] Remove the `install:skills` script from `package.json` (it references `install-skills.sh` which is being deleted — leaving it would break the script).
...
- [x] Audit `skills/_shared/references/` for files that are only referenced by deleted skills. If any shared reference file is no longer `@`-referenced by any remaining skill or agent, either delete it or note it as tech debt. Known orphan candidates:
...
- [x] Delete or update stale test harness files:
...
- [x] **Update `CLAUDE.md`** to reflect renamed and deleted skills:
