...
- [ ] `ls tools/dogfood/test-init.ts` — file does not exist; replaces `test-onboard.ts` functionality
...
- [ ] Write `tools/dogfood/test-init.ts` to replace `test-onboard.ts` with equivalent functional coverage (not just discoverability). `test-onboard.ts` tests actual onboarding behavior with generated fixtures — `test-init.ts` must include these functional tests for both modes:
