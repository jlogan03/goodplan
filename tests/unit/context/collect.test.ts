			...
			contents: {
				...
				"01-data-layer": {
					...
					contents: {
						"slice.json": {
							type: "json",
							content: { name: "01-data-layer", status: "implementing", goal: "Implement data layer" },
						},
...
describe("resolveContentSource", () => {
	...
	it("resolves a directory source to all markdown children", () => {
		const source: ContentSource = { key: "architecture", path: "architecture", sourceType: "directory" };
		const entries = resolveContentSource(fixture, source, rt);
		expect(entries).toHaveLength(3);
	});
	...
	it("returns empty for missing paths", () => {
		const source: ContentSource = { key: "missing", path: "does/not/exist.md", sourceType: "markdown" };
		const entries = resolveContentSource(fixture, source, rt);
		expect(entries).toHaveLength(0);
	});
