									...
									contents: {
										"slice.json": {
											...
											content: {
												...
												epic: "my-epic",
												status: "implementing",
												goal: "Implement data layer with Zod",
												deferred: [],
												refinement: null,
												created: "2026-03-20T00:00:00Z",
												updated: "2026-03-20T12:00:00Z",
										...
										"learnings.jsonl": {
											...
											content: [
												{
													category: "domain",
													summary: "JSONL is merge-friendly",
													file: "learnings/jsonl-is-merge-friendly.md",
													tags: ["data"],
													source: "epics/my-epic/slices/01-data-layer",
													rollup: false,
													rollupTo: [],
												},
											],
...
describe("startContext", () => {
	describe("plan phase (slice target)", () => {
		it("returns references without budget", () => {
			const bundle = startContext(state, "plan", {
				type: "slice",
				name: "01-data-layer",
				epic: "my-epic",
			});
		...
		it("inlines content with budget", () => {
			const bundle = startContext(
				state,
				"plan",
				{ type: "slice", name: "01-data-layer", epic: "my-epic" },
				{ inlineBudget: 50000 },
			);
			expect(Object.keys(bundle.inline).length).toBeGreaterThan(0);
			// Entity goal (slice.json) should be first priority — always inlined
			expect(bundle.inline["epics/my-epic/slices/01-data-layer/slice.json"]).toBe(
				"Implement data layer with Zod",
			);
		});
