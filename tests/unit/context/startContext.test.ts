...
const state: ProjectState = {
	...
	contents: {
		...
		epics: {
			type: "directory",
			contents: {
				"overview.json": { type: "json", content: { items: [{ name: "my-epic", status: "activated", created: "2026-03-20T00:00:00Z", completed: null }] } },
				"my-epic": {
					type: "directory",
					contents: {
						"epic.json": {
		...
		slices: {
			type: "directory",
			contents: {
				"overview.json": { type: "json", content: { items: [{ name: "01-data-layer", status: "implementing", created: "2026-03-20T00:00:00Z", completed: null }] } },
				"01-data-layer": {
					type: "directory",
					contents: {
						"slice.json": {
						...
						"learnings.jsonl": {
							type: "jsonl",
							content: [
								{ category: "domain", summary: "JSONL is merge-friendly", detail: "One record per line", tags: ["data"], source: "slices/01-data-layer", rollup: false, rollupTo: [] },
							],
						},
					},
				} satisfies DirectoryEntry,
...
describe("startContext", () => {
	describe("plan phase (slice target)", () => {
		it("returns references without budget", () => {
			const bundle = startContext(state, "plan", { type: "slice", name: "01-data-layer" });
		...
		it("inlines content with budget", () => {
			const bundle = startContext(state, "plan", { type: "slice", name: "01-data-layer" }, { inlineBudget: 50000 });
			expect(Object.keys(bundle.inline).length).toBeGreaterThan(0);
			// Entity goal (slice.json) should be first priority — always inlined
			expect(bundle.inline["slices/01-data-layer/slice.json"]).toBe("Implement data layer with Zod");
		});
	...
	describe("refinement phase (slice target)", () => {
		it("prioritizes plan content first", () => {
			const bundle = startContext(state, "refinement", { type: "slice", name: "01-data-layer" }, { inlineBudget: 50000 });
			const inlineKeys = Object.keys(bundle.inline);
			expect(inlineKeys[0]).toBe("slices/01-data-layer/plan.md");
		});
	});

	describe("implementation phase (slice target)", () => {
		it("prioritizes refined plan first", () => {
			const bundle = startContext(state, "implementation", { type: "slice", name: "01-data-layer" }, { inlineBudget: 50000 });
			const inlineKeys = Object.keys(bundle.inline);
			expect(inlineKeys[0]).toBe("slices/01-data-layer/plan-refined.md");
		});
	});
	...
	describe("refine-slices phase (epic target)", () => {
		it("includes slice definitions", () => {
			...
			// Should expand slices directory to find markdown entries
			expect(bundle.inline["slices/01-data-layer/plan.md"]).toBeDefined();
		});
	});

	describe("complete phase", () => {
		it("includes entity goal and architecture", () => {
			const bundle = startContext(state, "complete", { type: "slice", name: "01-data-layer" }, { inlineBudget: 50000 });
			expect(bundle.inline["slices/01-data-layer/slice.json"]).toBe("Implement data layer with Zod");
		});
	});
	...
	describe("budget edge cases", () => {
		it("with tiny budget, first entry is still inlined", () => {
			const bundle = startContext(state, "plan", { type: "slice", name: "01-data-layer" }, { inlineBudget: 1 });
