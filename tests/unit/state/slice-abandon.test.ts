...
describe("reduce — ABANDON_SLICE", () => {
	it("abandons from created status", () => {
		...
		const result = reduce(s, { type: "ABANDON_SLICE", epic: "e1", slice: "s1", ts: TS2, reason: "Not needed" });
	...
	it("abandons from planning status", () => {
		...
		const result = reduce(s, { type: "ABANDON_SLICE", epic: "e1", slice: "s1", ts: TS2, reason: "Changed direction" });
		expect(isStateError(result)).toBe(false);
		expect(getJson<Slice>(result as ProjectState, "epics/e1/slices/s1/slice.json")!.status).toBe("abandoned");
	});
	...
	it("abandons from implementing status", () => {
		...
		const result = reduce(s, { type: "ABANDON_SLICE", epic: "e1", slice: "s1", ts: TS2, reason: "Blocked" });
		expect(isStateError(result)).toBe(false);
		expect(getJson<Slice>(result as ProjectState, "epics/e1/slices/s1/slice.json")!.status).toBe("abandoned");
	});
	...
	it("completed timestamp is set on abandoned slice overview item", () => {
		const s = initWithSlice();
		const result = reduce(s, { type: "ABANDON_SLICE", epic: "e1", slice: "s1", ts: TS2, reason: "Done" }) as ProjectState;
	...
	it("clears activeSlice if this was the active slice", () => {
		...
		const result = reduce(s, { type: "ABANDON_SLICE", epic: "e1", slice: "s1", ts: TS2, reason: "Cancel" }) as ProjectState;
	...
	it("does not clear activeSlice if another slice is active", () => {
		...
		const result = reduce(s, { type: "ABANDON_SLICE", epic: "e1", slice: "s2", ts: TS2, reason: "Skip" }) as ProjectState;
	...
	it("rejects from completed (terminal)", () => {
		...
		const result = reduce(s, { type: "ABANDON_SLICE", epic: "e1", slice: "s1", ts: TS2, reason: "Too late" });
	...
	it("rejects from abandoned (terminal)", () => {
		let s = initWithSlice();
		s = reduce(s, { type: "ABANDON_SLICE", epic: "e1", slice: "s1", ts: TS, reason: "First" }) as ProjectState;

		const result = reduce(s, { type: "ABANDON_SLICE", epic: "e1", slice: "s1", ts: TS2, reason: "Second" });
		expect(isStateError(result)).toBe(true);
		expect((result as StateError).code).toBe("STATE_INVALID_TRANSITION");
	});
	...
	it("appends activity log entry with reason", () => {
		const s = initWithSlice();
		const result = reduce(s, { type: "ABANDON_SLICE", epic: "e1", slice: "s1", ts: TS2, reason: "Changed plan" }) as ProjectState;
