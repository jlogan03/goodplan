...
const _TS3 = "2026-01-03T00:00:00.000Z";
...
describe("reduce — COMPLETE_SLICE", () => {
	it("happy path: completes slice with deferred, learnings, architecture deltas", () => {
		...
		// Architecture deltas
		const deltas = getJsonl<ArchitectureDelta>(
			newState,
			"epics/e1/slices/s1/architecture-deltas.jsonl",
		);
	...
	it("epicComplete: all siblings completed (detected via overview)", () => {
		...
		const epicItem = overview?.epics.find((i) => i.name === "e1");
		const allDone = epicItem?.slices.every(
		...
		const result = reduce(s, {
			...
			learnings: [
				...
				},
				{
					category: "didnt-work",
					summary: "ProjectOnly",
					file: "learnings/project-only.md",
					tags: [],
					source: src,
					rollup: true,
					rollupTo: ["project"],
