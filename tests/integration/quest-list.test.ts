...
describe("quest:list pagination", () => {
	...
	it("limits items with --limit", async () => {
		await withFixture("pagination", ({ bin, env }) => {
			const result = runCommand(bin, ["quest:list", "--limit", "3", "--json"], { env });
			expect(result.exitCode).toBe(0);
	...
	it("skips items with --offset", async () => {
		await withFixture("pagination", ({ bin, env }) => {
			const result = runCommand(bin, ["quest:list", "--offset", "4", "--json"], { env });
			expect(result.exitCode).toBe(0);
			const data = result.json as {
				items: unknown[];
				total: number;
	...
	it("combines --limit and --offset", async () => {
		await withFixture("pagination", ({ bin, env }) => {
			const result = runCommand(bin, ["quest:list", "--offset", "1", "--limit", "2", "--json"], {
				env,
			});
			expect(result.exitCode).toBe(0);
			const data = result.json as {
			...
			expect(firstItem?.name).toBe("quest-two");
		});
	});

	it("--offset alone without --limit", async () => {
		await withFixture("pagination", ({ bin, env }) => {
			const result = runCommand(bin, ["quest:list", "--offset", "3", "--json"], { env });
			expect(result.exitCode).toBe(0);
			const data = result.json as {
				items: unknown[];
				total: number;
	...
	it("shows pagination footer in human mode", async () => {
		await withFixture("pagination", ({ bin, env }) => {
			const result = runCommand(bin, ["quest:list", "--limit", "3"], { env });
			expect(result.exitCode).toBe(0);
	...
	it("quiet mode produces no output with pagination", async () => {
		await withFixture("pagination", ({ bin, env }) => {
			const result = runCommand(bin, ["quest:list", "--limit", "2", "--quiet"], { env });
			expect(result.exitCode).toBe(0);
	...
	it("combines pagination with --query", async () => {
		await withFixture("pagination", ({ bin, env }) => {
			const result = runCommand(
				bin,
				["quest:list", "--limit", "2", "--json", "--query", ".items | length"],
				{ env },
			);
			expect(result.exitCode).toBe(0);
	...
	it("produces validation error for invalid --limit with exit code 2", async () => {
		await withFixture("pagination", ({ bin, env }) => {
			const result = runCommand(bin, ["quest:list", "--limit", "-1", "--json"], { env });
