...
- [Archive Convention (REMOVED)](#archive-convention-removed)
...
Created by `/create-epic`. Starts without `__active__` prefix. Renamed to `__active__<name>/` by CLI via `epic:activate` upon approval.
...
└── <name>/                          # all epics use bare names; status via CLI
    ...
    ├── architecture/                # target architecture (created by CLI via epic:activate from proposal upon approval)
...
> **Skills: use `gp epic:show --epic <name> --json` → `.status` for the authoritative state.** The file-existence rules below are the CLI's internal logic and must NOT be replicated by skills (per cli-interaction.md §3 and §7). They are documented here for reference only.

File-existence states (CLI-internal). Check in order — **first match wins**.
...
> The "Human-Readable State" column uses descriptive names from the file-existence table above. The "CLI Status" column shows the actual `.status` value from `epic:show --json`. **Skills should match on CLI Status values.**

### First Epic Transitions

| Human-Readable State | CLI Status | Trigger | To CLI Status | Skill |
|---|---|---|---|---|
| (none) | — | `/gp:create-epic` | `created` | `/gp:create-epic` |
| ready-for-exploration | `created` | `/gp:explore` | `exploring` | `/gp:explore` |
| ready-for-exploration | `created` | user skips explore | `explored` | writes `explore-skipped.md` |
| exploring | `exploring` | user marks done | `explored` | `/gp:explore` writes `explore-complete.md` |
| needs-architecture | `explored` → `defining-architecture` | `/gp:create-epic` | `architecture-refined` | `/gp:create-epic` writes to `architecture/` |
| needs-slice-planning | `architecture-refined` → `defining-slices` | `/gp:create-epic` | `slices-refined` | `/gp:create-epic` |
| executing-slices | `activated` | all slices complete | `activated` | check slice statuses |
| needs-completion | `activated` (all slices done) | `/gp:complete-epic` | `completed` | `/gp:complete-epic` |
| any | any | user abandons | `abandoned` | writes `abandoned.md` |
...
| ready-for-exploration | `created` | user skips explore | `explored` | writes `explore-skipped.md` |
| exploring | `exploring` | user marks done | `explored` | `/gp:explore` writes `explore-complete.md` |
| needs-architecture-proposal | `explored` → `defining-architecture` | `/gp:create-epic` | `architecture-refined` | `/gp:create-epic` writes to `architecture-proposal/` |
| needs-slice-planning | `architecture-refined` → `defining-slices` | `/gp:create-epic` | `slices-refined` | `/gp:create-epic` |
| proposal-pending | `slices-refined` | `/gp:start-epic` | `activated` | `/gp:start-epic` → `epic:activate` |
| executing-slices | `activated` | all slices complete | `activated` | check slice statuses |
| needs-completion | `activated` (all slices done) | `/gp:complete-epic` | `completed` | `/gp:complete-epic` |
| any | any | user abandons | `abandoned` | writes `abandoned.md` |
...
| **Epic** | `epics/<name>/architecture/` | Target state — where the epic is headed | `/gp:create-epic` (first init) or CLI via `epic:activate` (subsequent, from proposal) |
...
| `/gp:start-epic` | Reads (baseline) | Reads proposal; `approved.md` and `architecture/` from proposal are written by CLI via `epic:activate` |
...
| `approved.md` | CLI via `epic:activate` | State machine checks | — |
| `architecture/` (subsequent init) | CLI via `epic:activate` (from proposal upon approval) | Same as first init | `/gp:create-epic` (refinement) |
