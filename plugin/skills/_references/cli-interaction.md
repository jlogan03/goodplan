...
6. [State Orientation](#6-state-orientation)
   - [Conditions Loading](#conditions-loading)
...
The `gp` binary is on PATH (automatically added by Claude Code from the plugin's `bin/` directory). Use `gp` directly for all invocations. Skills conventionally store the binary name as `$GP` in their Step 0 — shared references use bare `gp` since they don't define the variable.
...
gp slice:plan --slice my-slice --json
...
```json
{
  ...
  "newStatus": "planning",
  "paths": {
    "plan": "/abs/path/to/.goodplan/epics/my-epic/slices/my-slice/plan.md",
    "research": "/abs/path/to/.goodplan/epics/my-epic/slices/my-slice/research/"
  }
}
```

**Note:** `paths` values may be file paths or directory paths depending on the phase. Check the actual CLI response.
...
Response is a flat `ContextBundle`: `{ inline: {...}, references: [...], decisions: [...], learnings: [...] }`. No wrapping `context` key, no `paths` field. Paths for where to write come from the mutation command that began the phase (e.g., `slice:plan` returns `paths`), not from `start-*`.
...
> **Deprecation:** The `state.md` format is obsolete. Skills should use the CLI commands above instead.
...
Some skills need to evaluate forward-compat conditions on decisions and learnings — fields that signal when a decision should be reconsidered or a learning may no longer apply. Load them with:
...
Filter client-side by `entityPath` prefix matching the current scope (e.g., `epics/EPIC_NAME`, `quests/QUEST_NAME`). Look for non-empty `reconsiderWhen` (decisions) or `validUntil` (learnings). If entries lack these fields entirely (forward compat — schema not yet updated), skip condition evaluation — do not error. Pass matched conditions to sub-agents for evaluation; surface any triggered conditions to the user.
...
`--inline` accepts bare form (`--inline`, uses default budget) or budget form (`--inline=<bytes>`, caps inlined content at the specified byte count).
...
Optional fields:
- `deferred` (array) — items deferred to other slices: `[{ "description": "...", "targetSlice": "...", "targetEpic?": "..." }]`
...
| `STATE_EPIC_ALREADY_ACTIVE` | 3 | Another epic is active | Complete or abandon the active epic first |
| `STATE_MISSING_VERIFICATIONS` | 3 | No verification criteria defined on epic | Add verification criteria before activation |
| `STATE_ALREADY_INITIALIZED` | 3 | `.goodplan/` already exists | Project is already initialized; proceed with other commands |
| `STATE_CONTENT_MISSING` | 3 | Required file not found at CLI-managed path (e.g., `plan.md`, `plan-refined.md`) | Copy the file to the correct path and retry |
| `STATE_SLICE_NOT_READY` | 3 | Previous slices not in terminal status | Complete or abandon previous slices first |
| `STATE_MAX_ROUNDS_REACHED` | 3 | Refinement round limit exceeded without passing | Use `--override` to force advancement, or improve scores |
| `STATE_VERIFICATION_FAILED` | 3 | `verificationPassed` is `false` in completion payload | Only submit with `verificationPassed: true`; handle quality signals via learnings |
...
```bash
gp quest:plan --quest fix-logging --json
