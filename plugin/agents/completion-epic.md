...
- **Verification criteria** — the epic's verification criteria array from `gp epic:show` (assess each in Step 6)
...
Write consolidated cross-slice learnings to `<epic-path>/completion/consolidated-learnings.md` (human-readable prose).

Additionally, return each learning as a structured entry in the `learnings` array of your return JSON. Each entry must match:
```json
{
  ...
  "tags": ["tag1", "tag2"],
  "rollupTo": ["project"],
  "validUntil": ["optional condition — e.g., 'schema version changes'"]
}
```

`validUntil` is optional — include when a learning has a known expiration condition.
...
| Decision records | Decisions that affect the whole project | Recommend in `recommendations` array — the orchestrator creates via `gp decision:create` |
...
### 6. Assess Verification Criteria

If the orchestrator included verification criteria in your task prompt, assess each criterion against the epic's completion state and artifacts. For each criterion, determine whether it has been met and provide a non-empty `notes` string explaining why. Return assessments in the `verificationAssessments` array (one entry per criterion, in order matching the input array).
...
```json
{
  ...
  "learnings": [
    { "category": "worked", "summary": "...", "detail": "...", "tags": ["..."], "rollupTo": ["project"], "validUntil": [] }
  ],
  "recommendations": [
    ...
    { "type": "decision", "id": "kebab-case-id", "domain": "architecture|tooling|etc", "title": "...", "summary": "...", "priority": "medium" }
  ],
...
**Partial** (use when some inputs are missing but meaningful analysis was still produced — e.g., some slices had no learnings, architecture files incomplete):
```json
{
  "status": "PARTIAL",
