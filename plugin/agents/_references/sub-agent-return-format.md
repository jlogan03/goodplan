...
| implement-phase | `filesWritten`, `lifecycle` | Changed files + granular TDD lifecycle results: `{ redWritten: boolean, redFailed: boolean, redFailureEvidence: string, greenPassed: boolean, greenEvidence: string, hasUnexpectedPass: boolean, details: string }`. Orchestrator emits only the chunk events the agent completed (enables partial-completion resume). |
...
| pressure-test-phase | `filesWritten`, `findings`, `triggeredConditions` | `findings: [{ class: string, severity: string, description: string, subsystem: string, recommendation: string, evidence: string }]` across 5 failure-mode classes. `triggeredConditions` for decisions/learnings whose conditions were met during analysis. |
| verifier-phase | `filesWritten`, `verificationEvidence` | `verificationEvidence: { chunkId: string, checksRun: [{ check: string, passed: boolean, output: string }], overallPassed: boolean }`. Independent chunk verification — orchestrator uses `overallPassed` to decide `chunk-verify` vs `chunk-unverifiable`. |
| completion-side-quest | `filesWritten`, `proposals`, `triageResults` | `proposals: [{ name, goal, scope, rationale, sourceFindings }]` — structured side quest definitions for CLI creation. `triageResults: [{ findingId, disposition: "promote"\|"merge"\|"cull", reason }]`. Orchestrator deduplicates proposals against completion-slice recommendations. |
