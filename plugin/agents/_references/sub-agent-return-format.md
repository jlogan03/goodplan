...
| verifier | `filesWritten`, `verificationEvidence` | `verificationEvidence: { chunkId: string, checksRun: [{ check: string, passed: boolean, output: string }], overallPassed: boolean }`. Independent chunk verification — orchestrator uses `overallPassed` to decide `chunk-verify` vs `chunk-unverifiable`. |
